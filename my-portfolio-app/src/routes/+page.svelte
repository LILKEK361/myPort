<script lang="ts">
    ...
    function onekeydown(e) {
        if (e.code === "Enter") {
            ...
            terminalOutput.innerHTML += `<div class="flex w-100% h-10%">
...
<div class="container h-full mx-auto flex justify-center items-center bg-surface-900 text-green-700">
    <div class=" w-[90%] h-[90%]  border-2 border-green-400 ">
        <div class="relative min-h-[95%]  max-h-[95%] bg-surface-900 pt-5 pb-5 overflow-hidden"
             bind:this={terminalOutput}></div>
        <div class="h-[5%] w-[100%] bg-surface-600 flex border-2 border-green-400">
            <div class="w-[10%] text-center ml-[15px] mr-[15px] h-[50%]">myportfolio@user:</div>
            <input class="w-[90%] bg-surface-600 pl-2" bind:this={terminalInput} on:keydown={onekeydown}>
        </div>
    </div>
</div>
